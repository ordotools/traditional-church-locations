// Default extractor, used for any site without a dedicated adapter in
// ../registry.js. Real-world directories (see ROADMAP.md) are messy: titles
// and addresses run together, some entries have no street address at all,
// and organization codes are unlabeled abbreviations. This is a heuristic,
// human-reviewed extractor, not a general address parser — every candidate
// it produces goes through the admin review queue before it becomes a pin.
...
function extract($) {
  ...
  return tableResults && tableResults.length ? tableResults : extractFromLines($);
}

module.exports = { extract, US_STATES, cleanTitle };
